...
func NewLogger(filePath string, level zapcore.Level, maxSize int,
	...
	return zap.New(core, zap.AddCaller(), zap.Development(),
		...
		zap.Fields(zap.String("serviceName2", serviceName)))
}

/**
 * zapcore构造
 */
func newCore(filePath string, level zapcore.Level, maxSize int,
	maxBackups int, maxAge int, compress bool) zapcore.Core {
	//日志文件路径配置2
